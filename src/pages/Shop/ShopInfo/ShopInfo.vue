...
<script>
  ...
  export default {
    ...
    methods:{
      _initScroll(){
        new BScroll(this.$refs.info, {
          click: true//默认是垂直滑动
        })
        //动态计算ul的宽度（建立在已有数据的基础上）
        const ul = this.$refs.ul
        const liWidth = 120
        const space = 6
        const imgCount = this.info.pics.length
        ul.style.width = (liWidth + space) * imgCount - space + 'px'

        new BScroll(this.$refs.pics, {
          click: true,
          scrollX: true //设置为水平滑动
        })
      }
    },
    watch: {//在当前路由ShopInfo进行刷新，使用watch监听
      info(){//刷新后初始化显示 --> 更新数据
        this.$nextTick(()=>{
          this._initScroll()
        })
